...
public class AuthenticationService {
    ...
    public TokensDto refreshToken(final RefreshTokenDto dto) {
        final var user = findUserByRefreshToken(dto.refreshToken());
        final var isTokenValid = jwtService.isTokenValid(dto.refreshToken(), user.getUsername());

        if (isTokenValid) {
            final var accessToken = jwtService.generateAccessToken(user.getUsername());

            user.setToken(accessToken);
            repository.updateUser(user);

            return new TokensDto(accessToken, dto.refreshToken());
        }

        throw new UserAuthenticationException();
    }

    private User findUserByRefreshToken(final String refreshToken) {
        return jwtService
                .extractUsername(refreshToken)
                .map(username -> repository
